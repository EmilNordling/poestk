...
// 	{
// 		name: 'aColor',
// 		data: (index, total) => getHSL(begin + (index / total) * 0.2, 0.6 + getRandom(0.1), 0.6 + getRandom(0.1)),
// 		size: 3,
// 	},
// 	{
// 		name: 'aOffset',
...
interface AttributeProps {
	name: string;
	size: number;
	data: (index: number, total: number) => [number, number, number];
}

/**
 * @description Passive Tree Renderer
 */
class Ptr {
	private canvasRef: HTMLCanvasElement | undefined;
	private renderer: Phenomenon | undefined;

	constructor(private readonly matrix: Matrix) {}

	public createInstance(canvas: HTMLCanvasElement): void {
		if (this.matrix.nodes.size === 0) return;

		this.canvasRef = canvas;

		this.renderer = new Phenomenon({
			canvas,
			settings: {
		...
		const circleGeometry = () => {
			...
			for (let i = 0; i <= 360; i += 4) {
				const degrees = (i * Math.PI) / 180;

				vertices.push(Math.sin(degrees), Math.cos(degrees), 0, 0);
			}
		...
		this.renderer.add('node', {
			attributes,
			uniforms,
			vertex,
			fragment,
			multiplier: this.matrix.nodeIds.length,
		});

		// this.renderer.add('starling', {
		// 	attributes,
		// 	multiplier,
		// 	uniforms,
		// 	vertex,
